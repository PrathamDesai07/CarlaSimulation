...
class GlobalRoutePlanner:
    ...
    def __init__(self, wmap, sampling_resolution=None):
        # type: (carla.Map, float) -> None
        # Accept either (wmap, res) or a GlobalRoutePlannerDAO as first arg
        if hasattr(wmap, 'get_map'):
            dao = wmap
            self._wmap = dao.get_map()
            self._sampling_resolution = dao.get_sampling_resolution()
        else:
            self._wmap = wmap
            self._sampling_resolution = sampling_resolution or 2.0
    ...
    def setup(self):
        """Compatibility shim — graph is built in __init__. Subclass override point."""
        pass
